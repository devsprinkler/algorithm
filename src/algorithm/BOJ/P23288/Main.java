...
// 지도의 좌표는 (r, c)로 나타내며, r는 북쪽으로부터
// 떨어진 칸의 개수, c는 서쪽으로부터 떨어진 칸의 개수이다.
// 가장 왼쪽 위에 있는 칸의 좌표는 (1, 1)이고,
// 가장 오른쪽 아래에 있는 칸의 좌표는 (N, M)이다.
// 이 지도의 위에 주사위가 하나 놓여져 있으며,
// 주사위의 각 면에는 1보다 크거나 같고,
// 6보다 작거나 같은 정수가 하나씩 있다.
...
// 주사위의 아랫면에 있는 정수 A와 주사위가 있는 칸 (x, y)에 있는 정수 B를
// 비교해 이동 방향을 결정한다.
// A > B인 경우 이동 방향을 90도 시계 방향으로 회전시킨다.
// A < B인 경우 이동 방향을 90도 반시계 방향으로 회전시킨다.
// A = B인 경우 이동 방향에 변화는 없다.
// 칸 (x, y)에 대한 점수는 다음과 같이 구할 수 있다.
// (x, y)에 있는 정수를 B라고 했을때, (x, y)에서 동서남북
...
//
// 이 문제의 예제 1부터 7은 같은 지도에서 이동하는 횟수만
// 증가시키는 방식으로 구성되어 있다. 예제 8은 같은 지도에서 이동하는
// 횟수를 매우 크게 만들었다.